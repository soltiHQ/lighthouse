...
// Service implements credential-related use-cases on top of storage contracts.
type Service struct {
...
func New(store storage.Storage, logger zerolog.Logger) *Service {
	if store == nil {
		panic("credentials.Service: store is nil")
	}
	return &Service{
...
func (s *Service) SetPassword(ctx context.Context, req SetPassword) error {
